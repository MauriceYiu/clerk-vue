				...
				<div id="right_goods_scroll">
					<div class="goods_item oneLevel">
						...
						<div class="goods_li" v-for="(item,ind) in goodsData[curIdx].products">
	                        ...
	                        <div class="goodMsg" @click="addProduct(item.id, true,null,false)">
  	...
  	methods: {
           ...
           let _this = this;
            ...
            wx.scanQRCode({
                ...
                success: function (res) {
                    ...
                    _this.$http.put(BASE_URL+'/v1/clerk/scanPayments/' + _this.$uuid(), data).then((res)=>{
                        _this.payCode = '';
                        if ( res.status === 202 ) {
                            let overTimeNum = 0;
        ...
        testPay(){
            ...
            let data = {
                    ...
                    //     return;
                    // }
                    this.$http.put(BASE_URL+'/v1/clerk/scanPayments/' + this.$uuid(), data).then((res)=>{
                        this.testPayCode = '';
                        if ( res.status === 202 ) {
                            let overTimeNum = 0;
        ...
        closeCodeImg: function () {
  		...
  		// 添加商品
  		addProduct: function ( productId, btn, num ,ISmsg) {
            if(!this.canClick){
                // this.$message({
                //     message: "点击不能过快",
                //     showClose: true,
